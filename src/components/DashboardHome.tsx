    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-700 p-6 sm:p-8 text-white shadow-lg">
        <div className="relative z-10 space-y-2">
          ...
          <p className="text-sm sm:text-base text-blue-100 max-w-2xl font-light">
          ...
          <div className="h-64 flex flex-col justify-between">
            <div className="flex-1 flex items-end gap-6 px-4">
              {charts.mahasiswaPerProdi.map((item: any, idx: number) => {
                ...
                const barColors = ['bg-blue-600', 'bg-blue-500', 'bg-sky-500', 'bg-indigo-500'];
          ...
          <div className="flex flex-col gap-3 px-1">
            {(() => {
              ...
              return charts.gradeDistribution.map((grade: any, index: number) => {
                ...
                const barColors = ['bg-blue-600', 'bg-blue-500', 'bg-indigo-500', 'bg-sky-500', 'bg-amber-500', 'bg-orange-500', 'bg-rose-500'];
                ...
                <div key={item.name} className="space-y-1.5">
                  ...
                  <div className="w-full bg-slate-100 dark:bg-slate-800 h-4 rounded-lg overflow-hidden flex">
                    <div
                      ...
                      className="bg-gradient-to-r from-blue-500 to-indigo-500 h-full rounded-l-lg"
                    />
                  </div>
                </div>
              );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              {currentRole === 'Administrator' && (
                <button
                  ...
                  className="text-xs px-2.5 py-1 rounded-md bg-blue-600 text-white font-semibold hover:bg-blue-700 transition-colors"
                >
            ...
            {showAnnounceForm && (
              <form onSubmit={handleAddAnnouncement} className="mb-4 p-3 border border-blue-100 dark:border-blue-900/30 rounded-xl bg-blue-50/20 dark:bg-blue-950/10 space-y-2">
                <input
                  type="text"
                  placeholder="Judul pengumuman"
                  value={newTitle}
                  onChange={(e) => setNewTitle(e.target.value)}
                  className="w-full text-xs p-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-white focus:outline-blue-500"
                  required
                />
                <textarea
                ...
                <button
                  type="submit"
                  className="text-xs px-3 py-1.5 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700"
                >
                  Publikasi
                </button>
              </form>
            ...
            <div className="space-y-3.5 max-h-80 overflow-y-auto">
              {announcements.map((announce) => (
                <div key={announce.id} className="p-4 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30">
                  ...
                  <span className="text-[10px] font-semibold text-blue-600 dark:text-blue-400">Oleh: {announce.author}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              <div key={log.id} className="flex gap-3 text-xs border-b border-slate-50 dark:border-slate-800/60 pb-3 last:border-0 last:pb-0">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-slate-600 dark:text-slate-400 mt-0.5">
                    <span className="font-semibold text-blue-600 dark:text-blue-400 mr-1">[{log.aktivitas}]</span>
                    {log.deskripsi}
                  </p>
